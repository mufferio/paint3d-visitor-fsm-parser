...
public class PaintFileParserTest {
    ...
    private void doParserTestCase(String fileName, String message, String expectedErrorMessage) throws IOException {
        ...
        BufferedReader lineInput = new BufferedReader(new FileReader(fullFileName));
        ...
        if (expectedErrorMessage.equals("")) {
            assertTrue(retVal, fileName + ": Returns true for basic file with no spaces");
            assertEquals("", errorMessage, fileName + ": No error message");
        } else {
            String reportedLine = "", expectedLine = "";

            assertFalse(retVal, fileName + ": Returns false for basic incorrect file format");
            ...
            assertEquals(expectedLine, reportedLine, fileName + ": Error Message");
        }
        lineInput.close();
    }
